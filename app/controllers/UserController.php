...
class UserController extends BaseController {
    ...
    protected $access = array(
        'index'          => null,
        'create'         => null,
        'signup'       => array('Super Admin', 'Admin'),
        'store'          => null,
        'show'           => null,
        'getInfo'        => null,
	...
	{ 
        foreach (User::all() as $user){
            if($user->last_login) {
                $user->status = (intval($user->last_login->diffInDays()) <= '7') ? 'active' : 'inactive';
            }
            else {
        ...
        return View::make('users.index')->with('users', $allUsers);
	}
	...
	public function create()
	{
        ...
        return View::make('users.create')->with('allGroups', $allGroups);
	}
    ...
    {
        $data = Input::all();
        preg_match_all('!\d+!', $data['phone'], $tmp);
        $data['phone'] = implode('', $tmp[0]);
        preg_match_all('!\d+!', $data['fax'], $tmp);
        $data['fax'] = implode('', $tmp[0]);

        // Form Processing
        $result = $this->userForm->create( $data );

        if( $result['success'] )
        {
            // Success!
            Session::flash('success', trans('users.created2'));

        } else {
            Session::flash('error', trans('users.notcreated'));
        }

        return Redirect::action('UserController@index');
        //return View::make('users.create')->with('inputs', $data);
    }
